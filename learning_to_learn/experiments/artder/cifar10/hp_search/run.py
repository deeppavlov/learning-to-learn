...
meta_optimizer_build_kwargs = dict(
    regime='inference',
    additional_metrics=add_metrics,
    selection_application=sel_application,
)
...
for conf in confs:
    ...
    if sel_application == 'mean':
        other_hyperparameters = dict(
            ...
            num_sel=dict(
                hp_type='additional_placeholder',
                varying=conf['num_sel/value'],
            ),
            selection_size=dict(
                hp_type='additional_placeholder',
                varying=conf['selection_size/value'],
            ),
        )
    elif sel_application == 'shuffle':
        other_hyperparameters = dict(
            learning_rate=dict(
                varying=dict(
                    init=conf['learning_rate/init']
