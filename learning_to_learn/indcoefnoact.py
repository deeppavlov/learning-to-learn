...
class IndCoefNoAct(Meta):
    ...
    def _create_optimizer_trainable_vars(self):
        total_ndims = sum(flatten(self._pupil_dims)) * self._pupil_net_size['num_unrollings']
        with tf.variable_scope('optimizer_trainable_variables'):
            matrix = tf.Variable(tf.ones([total_ndims]), name='coefs')
            bias = tf.Variable(tf.zeros([total_ndims]), name='batch')
            vars = [matrix, bias]
